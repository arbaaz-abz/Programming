...
class Kruskal {
	...
	public static void compute(int cost[][],int n) {
		//Make an 2-D array with three columns for u,v and cost between u and v
		int a[][] = new int[5*n][3];
		int k=0;
		for(int i=0;i<n;i++) {
			for(int j=0;j<n;j++){
		...
		int t[][] = new int[k][2];    //All Set of edges added MST
		int parent[] = new int[n];
		int count = 0;
		int sum = 0;
		for(int i=0; i<n; i++)
		{
			parent[i] = i; //initialize the disjoint sets: parent[ ] is the representative of the set
		}
        int u,v,i,j,pos,m=0;
        while(count != n-1)
		{
